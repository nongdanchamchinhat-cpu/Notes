...
export function LoginToOpenNotes() {
  return (
    <StepContainer
      onSubmit={(e) => {
        e.preventDefault();
    ...
    >
      ...
      <Text as="p" variant="body" sx={{ mt: 1 }}>
        ...
        <Link
          target="_blank"
          href="https://blog.opennotes.openlay.com/opennotes-is-going-open-source"
          sx={{ color: "primary", fontWeight: "bold" }}
        >
          we have completely open sourced OpenNotes.
        </Link>
      </Text>
      <Text as="p" variant="body" sx={{ mt: 1 }}>
        Yes, that&apos;s right. OpenNotes is now 100% open source under the
      ...
      <Text as="p" variant="body" sx={{ mt: 1 }}>
        ...
        <Code text="@opennotes/crypto" href={getPackageUrl("crypto")} /> — the
        main library for all cryptographic operations inside OpenNotes.
      </Text>
      <Text as="p" variant="body" sx={{ mt: 1 }}>
        Vericrypt will allow you to verify all encryption claims made by
        OpenNotes in a practical &amp; provable way right inside your browser.
      </Text>
      <Text
        as="p"
        variant="body"
      ...
      <Button
        ...
        onClick={() => window.open("https://app.opennotes.openlay.com/login", "_blank")}
      >
        Login to OpenNotes
      </Button>
    </StepContainer>
