...
import { useIsFeatureAvailable } from "@opennotes/common";
import { Color, Note } from "@opennotes/core";
import { strings } from "@opennotes/intl";
import { useThemeColors } from "@opennotes/theme";
import React, { useCallback, useEffect, useState } from "react";
import { View } from "react-native";
import Icon from "react-native-vector-icons/MaterialCommunityIcons";
import { opennotes } from "../../../e2e/test.ids";
...
const ColorItem = ({ item, note }: { item: Color; note: Note }) => {
  ...
  return (
    <Pressable
      ...
      testID={opennotes.ids.dialogs.actionsheet.color(item.colorCode)}
      key={item.id}
      onPress={toggleColor}
      style={{
        width: 35,
