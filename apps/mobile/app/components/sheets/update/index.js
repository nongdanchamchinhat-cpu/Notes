...
export const Update = ({ version: appVersion, fwdRef }) => {
  ...
  let notes = version?.notes
    ? version.notes.replace("Thank you for using OpenNotes!", "").split("- ")
  ...
  const GITHUB_URL =
    !version || !version.needsUpdate
      ? null
      : `https://github.com/openlay/opennotes/releases/download/${
          version.version
        }-android/opennotes-${getSupportedAbi()}.apk`;
  const GITHUB_PAGE_URL =
    !version || !version.needsUpdate
      ? null
      : `https://github.com/openlay/opennotes/releases/tag/${version.version}-android`;
