...
*/
import { Plan, SubscriptionPlan, SubscriptionPlanId } from "@opennotes/core";
...
const pricingPlans: PricingPlan[] = [
  ...
  },
  {
    ...
    productSkuList: ["opennotes.pro.5year"],
    trialSupported: true,
    recommended: true
  },
  {
    ...
    productSkuList: ["opennotes.believer.5year"],
    trialSupported: true
  }
];
...
const usePricingPlans = (options?: PricingPlansOptions) => {
  ...
  const [selectedProductSku, setSelectedProductSku] = useState<string>(
    options?.productId || "opennotes.pro.yearly"
  );
  ...
  async function getRegionalDiscount(plan: string, productId: string) {
    if (productId !== "opennotes.pro.yearly") {
      return;
    }
    try {
