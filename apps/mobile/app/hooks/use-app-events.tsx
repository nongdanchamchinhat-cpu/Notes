...
import { isFeatureAvailable } from "@opennotes/common";
import {
  EV,
  EVENTS,
...
const onAppOpenedFromURL = async (event: {
  ...
  try {
    if (url.startsWith("https://app.opennotes.openlay.com/account/verified")) {
      ...
      return;
    } else if (url.startsWith("https://app.opennotes.openlay.com/open_note?")) {
      ...
      url.startsWith("https://app.opennotes.openlay.com/open_notebook?") &&
      !event.isInitialUrl
    ) {
      const id = new URL(url).searchParams.get("id");
      if (id) {
      ...
      url.startsWith("https://app.opennotes.openlay.com/open_tag?") &&
      !event.isInitialUrl
    ) {
      const id = new URL(url).searchParams.get("id");
      if (id) {
      ...
      url.startsWith("https://app.opennotes.openlay.com/open_color?") &&
      !event.isInitialUrl
    ) {
      const id = new URL(url).searchParams.get("id");
      if (id) {
    ...
    } else if (url.startsWith("https://app.opennotes.openlay.com/open_reminder")) {
      const id = new URL(url).searchParams.get("id");
      if (id) {
        const reminder = await db.reminders.reminder(id);
        if (reminder) AddReminder.present(reminder);
      }
    } else if (url.startsWith("https://app.opennotes.openlay.com/new_reminder")) {
...
async function saveEditorState() {
  if (!editorState().movedAway) {
    ...
    OpenNotesModule.setAppState(state);
  } else {
    OpenNotesModule.setAppState("");
  }
}
