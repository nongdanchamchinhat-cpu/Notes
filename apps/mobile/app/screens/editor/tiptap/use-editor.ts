...
import { getFormattedDate } from "@opennotes/common";
import {
  ContentItem,
  ContentType,
  DeletedItem,
...
import { useThemeEngineStore } from "@opennotes/theme";
...
export const useEditor = (
  ...
  const sessionIdRef = useRef("opennotes-editor");
  ...
  const onLoad = useCallback(async () => {
    ...
    if (!state.current?.initialLoadCalled) {
      ...
      let noteId =
        url &&
        url.startsWith("https://app.opennotes.openlay.com/open_note?") &&
