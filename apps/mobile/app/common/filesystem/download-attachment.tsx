...
import { getFileNameWithExtension } from "@opennotes/core";
...
export async function downloadAttachments(attachmentIds: string[]) {
  ...
  const zipSourceFolder = `${cacheDir}/opennotes-attachments`;
  const zipOutputFile =
    Platform.OS === "ios"
      ? `${outputFolder}/opennotes-attachments-${Date.now()}.zip`
      : `${cacheDir}/opennotes-attachments.zip`;
  ...
  try {
    ...
    // If all goes well, zip the opennotes-attachments folder in cache.
    ...
    if (Platform.OS === "android") {
      ...
      const file = await ScopedStorage.createFile(
        outputFolder,
        `opennotes-attachments-${Date.now()}.zip`,
        "application/zip"
      );
