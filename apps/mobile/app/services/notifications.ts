...
import { getFormattedReminderTime } from "@opennotes/common";
import { isReminderActive, Reminder } from "@opennotes/core";
...
const onEvent = async ({ type, detail }: Event) => {
  ...
  if (
    ...
    Platform.OS === "android" &&
    notification?.id === "opennotes_note_input" &&
...
async function updateRemindersForWidget() {
  ...
  OpenNotesModule.setString(
    "appPreview",
    "remindersList",
    JSON.stringify(activeReminders)
  );
  OpenNotesModule.updateReminderWidget();
}
...
async function loadNote(id: string, jump: boolean) {
  if (!id || id === "opennotes_note_input") return;
  ...
  switch (id) {
    case "default":
      return await notifee.createChannel({
        id: "com.openlay.opennotes",
        name: "Default",
        vibration: false
      });
    case "silent":
      return await notifee.createChannel({
        id: "com.openlay.opennotes.silent",
        name: "Silent",
        vibration: false
      });
    case "vibrate":
    ...
    case "urgent":
      return await notifee.createChannel({
        id: "com.openlay.opennotes.urgent",
        name: "Urgent",
        description:
...
async function pinQuickNote() {
  ...
  get().then((items) => {
    const notification = items.find((n) => n.id === "opennotes_note_input");
