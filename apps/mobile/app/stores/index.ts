...
import { OpenNotesModule, ShortcutInfo } from "../utils/opennotes-module";
        ...
        case "note":
          {
            ...
            if (!note) {
              OpenNotesModule.removeShortcut(shortcut.id);
            } else if (note.title !== shortcut.title) {
              OpenNotesModule.updateShortcut(
                shortcut.id,
                "note",
                note.title,
                note.headline
...
export function initAfterSync(type: "full" | "send" = "send") {
  ...
  OpenNotesModule.getAllShortcuts()
    .then(syncShortcuts)
    .catch((e) => {
