...
mergedConfig.resolver = {
  ...
  nodeModulesPaths,
  extraNodeModules: {
    "@opennotes": path.join(__dirname, "../../packages"),
  ...
  resolveRequest: (context, moduleName, platform) => {
    ...
    if (moduleName === "@opennotes/core") {
      const result = require.resolve(moduleName); // gets CommonJS version
      return context.resolveRequest(context, result, platform);
    }

    if (moduleName === "@opennotes/common") {
      const result = require.resolve(moduleName); // gets CommonJS version
      return context.resolveRequest(context, result, platform);
    }

    if (moduleName === "@opennotes/logger") {
      const result = require.resolve(moduleName); // gets CommonJS version
      return context.resolveRequest(context, result, platform);
    }

    if (moduleName === "@opennotes/theme") {
      const result = require.resolve(moduleName); // gets CommonJS version
      return context.resolveRequest(context, result, platform);
    }
