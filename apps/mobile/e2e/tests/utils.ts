...
import { opennotes } from "../test.ids";
...
const Tests = {
  awaitLaunch: async () => {
    ...
    await waitFor(element(by.id(opennotes.ids.default.root)))
      .toBeVisible()
      //@ts-ignore
  ...
  async createNote(title?: string, _body?: string) {
    ...
    await Tests.fromId(opennotes.buttons.add).tap();
    if (title) {
  ...
  async navigate(screen: RouteName | ({} & string)) {
    let menu = Tests.fromId(opennotes.ids.default.header.buttons.left);
    await menu.waitAndTap();
    await Tests.fromText(screen as string).waitAndTap();
  },
  async openSideMenu() {
    await Tests.fromId(opennotes.ids.default.header.buttons.left).waitAndTap();
  },
  async prepare() {
    await device.disableSynchronization();
  ...
  async createNotebook(title = "Notebook 1", description = true) {
    ...
    const titleInput = Tests.fromId(
      opennotes.ids.dialogs.notebook.inputs.title
    );
    ...
    if (description) {
      await Tests.fromId(
        opennotes.ids.dialogs.notebook.inputs.description
