...
async function lockNote() {
  await TestBuilder.create()
    .waitAndTapById(opennotes.listitem.menu)
    .wait()
    .waitAndTapById("icon-lock-unlock")
    .wait()
    .isVisibleByText("Lock")
    .typeTextById(opennotes.ids.dialogs.vault.pwd, "1234")
    .typeTextById(opennotes.ids.dialogs.vault.pwdAlt, "1234")
...
async function openLockedNote(pwd = "1234") {
  await TestBuilder.create()
    .waitAndTapById(opennotes.ids.note.get(0))
    .wait()
    .addStep(async () => {
...
describe("VAULT", () => {
  ...
  it("Change vault password", async () => {
    await TestBuilder.create()
      ...
      .typeTextById(opennotes.ids.dialogs.vault.pwd, "1234")
      .typeTextById(opennotes.ids.dialogs.vault.changePwd, "2362")
  ...
  it("Delete vault", async () => {
    await TestBuilder.create()
      ...
      .pressBack(3)
      .isVisibleById(opennotes.listitem.menu)
      .run();
  });
  ...
  it("Delete vault with locked notes", async () => {
    await TestBuilder.create()
      ...
      .isVisibleByText("Create vault")
      .pressBack(3)
      .isNotVisibleById(opennotes.listitem.menu)
      .run();
  });
