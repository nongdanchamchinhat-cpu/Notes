...
import { initialize, logger as _logger, logManager } from "@opennotes/core";
import { LogMessage, NoopLogger, format } from "@opennotes/logger";
import { ZipFile } from "./streams/zip-stream";
import { createWriteStream } from "./stream-saver";
import { sanitizeFilename } from "@opennotes/common";
...
async function initializeLogger() {
  ...
  logger = _logger.scope("opennotes-web");
}
...
async function downloadLogs() {
  ...
  await new ReadableStream<ZipFile>({
    ...
    .pipeTo(await createWriteStream("opennotes-logs.zip"));
}
