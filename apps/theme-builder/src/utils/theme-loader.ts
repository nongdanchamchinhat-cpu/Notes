...
export async function loadThemeFromURL(url: string) {
  try {
    return loadThemeFromJSON(
      await fetch(`https://cors.opennotes.openlay.com/${url}`).then((res) => res.json())
    );
  } catch (err) {
    console.error(err);
    return null;
