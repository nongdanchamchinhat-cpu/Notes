...
import { Messages, setI18nGlobal } from "@opennotes/intl";
...
if (process.platform == "win32" && process.env.PORTABLE_EXECUTABLE_DIR) {
  ...
  const root = path.join(process.env.PORTABLE_EXECUTABLE_DIR, "OpenNotes");
...
async function createWindow() {
  ...
  mainWindow.webContents.session.setSpellCheckerDictionaryDownloadURL(
    "http://dictionaries.opennotes.openlay.com/"
  );
...
app.once("ready", async () => {
  ...
  if (app.runningUnderARM64Translation) {
    ...
    dialog.showMessageBoxSync({
      message:
        "OpenNotes detected that it is running under ARM64 translation. For the best performance, please download the ARM64 build of OpenNotes from our website.",
      type: "warning",
      buttons: ["Okay"],
