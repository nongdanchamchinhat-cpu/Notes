...
export const useAppStore = create<AppStore>((set) => ({
  ...
  async login(openNew = false) {
    ...
    const opennotes = await connectApi(openNew, () => {
      set({
        user: undefined,
        isLoggedIn: false,
        isLoggingIn: false,
    ...
    if (!opennotes) {
      ...
      throw new Error(
        "Please refresh the OpenNotes web app to connect with the Web Clipper."
      );
    }

    const user = await opennotes.login();
    const notes = await opennotes.getNotes();
    const notebooks = await opennotes.getNotebooks();
    const tags = await opennotes.getTags();
