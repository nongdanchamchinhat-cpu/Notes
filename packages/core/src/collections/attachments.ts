...
import { Cipher, DataFormat, SerializedKey } from "@opennotes/crypto";
      ...
      types.includes("all")
        ...
        : selector.filter.where((eb) => {
            ...
            if (types.includes("webclips"))
              filters.push(
                eb("mimeType", "==", `application/vnd.opennotes.web-clip`)
              );
    ...
    return this.collection.createFilter<Attachment>(
      (qb) =>
        qb
          ...
          .where("mimeType", "==", `application/vnd.opennotes.web-clip`),
      this.db.options?.batchSize
    );
  }
...
export function getOutputType(attachment: Attachment): DataFormat {
  if (attachment.mimeType === "application/vnd.opennotes.web-clip")
    return "text";
