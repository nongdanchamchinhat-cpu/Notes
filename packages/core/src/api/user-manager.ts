...
import { SerializedKeyPair, SerializedKey } from "@opennotes/crypto";
...
class UserManager {
        ...
        await http
          .post(
            `${constants.AUTH_HOST}${ENDPOINTS.token}`,
            {
              ...
              client_id: "opennotes",
              scope: "opennotes.sync offline_access IdentityServerApi",
              password: hashedPassword
            },
            token.access_token
          )
  ...
  async verifyPassword(password: string) {
    try {
      ...
      const cipher = await this.db.storage().encrypt(key, "opennotes");
      const plainText = await this.db.storage().decrypt({ password }, cipher);
      return plainText === "opennotes";
    } catch (e) {
      logger.error(e);
      return false;
    }
