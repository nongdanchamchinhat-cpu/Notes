...
description: OpenNotes allows you to backup all your notes data to a single backup file. Learn how you can backup your notes and restore them.
---
...
>
> Regardless of the folder you select, OpenNotes will create a folder "OpenNotes/backups" inside it and store all backup files there.
...
> info
>
> On the **web** app there is no way to automatically save backups to a folder, that is why OpenNotes only reminds the users when it's time to create a new backup.
...
At any point in time, you can restore a backup to recover lost data. However, **to restore a backup, you must be logged in to your OpenNotes account.** Backups created on one account can be restored on another OpenNotes account.
